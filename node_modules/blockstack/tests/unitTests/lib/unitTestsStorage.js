...
function runStorageTests() {
  (0, _tape2.default)('getFile unencrypted, unsigned', function (t) {
    ...
    var _proxyquire = (0, _proxyquire19.default)('../../../lib/storage', { // eslint-disable-line no-shadow
      './hub': { getOrSetLocalGaiaHubConnection: getOrSetLocalGaiaHubConnection, getFullReadUrl: getFullReadUrl }
        ...
        getFileImpl = _proxyquire.getFileImpl;

    _fetchMock2.default.get(fullReadUrl, fileContent);
    var options = { decrypt: false };
    getFileImpl(blockstack, path, options).then(function (file) {
  ...
  (0, _tape2.default)('getFile unencrypted, unsigned - multi-reader', function (t) {
    ...
    var appConfig = new _lib.AppConfig(['store_write'], 'http://localhost:8080');
    var blockstack = new _lib.UserSession({ appConfig: appConfig });
    ...
    var _proxyquire2 = (0, _proxyquire19.default)('../../../lib/storage', { // eslint-disable-line no-shadow
      './hub': { getOrSetLocalGaiaHubConnection: getOrSetLocalGaiaHubConnection, getFullReadUrl2: getFullReadUrl2 }
        ...
        getFileImpl = _proxyquire2.getFileImpl;
    ...
    getFileImpl(blockstack, path, optionsNameLookupUrl).then(function (file) {
      t.ok(file, 'Returns file content');
      t.same(JSON.parse(file), JSON.parse(fileContents));
    });
    ...
    getFileImpl(blockstack, path, optionsNoApp).then(function (file) {
      t.ok(file, 'Returns file content');
      t.same(JSON.parse(file), JSON.parse(fileContents));
    });
  });
  ...
  (0, _tape2.default)('encrypt & decrypt content', function (t) {
    ...
    blockstack.store.getSessionData().userData = {
      appPrivateKey: privateKey // manually set private key for testing

    };var content = 'yellowsubmarine';
    var ciphertext = blockstack.encryptContent(content);
    t.ok(ciphertext);
    var deciphered = blockstack.decryptContent(ciphertext);
  ...
  (0, _tape2.default)('encrypt & decrypt content -- specify key', function (t) {
    ...
    var ciphertext = blockstack.encryptContent(content, { publicKey: publicKey });
    t.ok(ciphertext);
    var deciphered = blockstack.decryptContent(ciphertext, { privateKey: privateKey });
    t.equal(content, deciphered);
  });
  ...
  (0, _tape2.default)('putFile unencrypted, not signed', function (t) {
    ...
    var _proxyquire3 = (0, _proxyquire19.default)('../../../lib/storage', {
      './hub': { getOrSetLocalGaiaHubConnection: getOrSetLocalGaiaHubConnection, uploadToGaiaHub: uploadToGaiaHub }
        ...
        putFileImpl = _proxyquire3.putFileImpl;

    var options = { encrypt: false };

    putFileImpl(blockstack, path, fileContent, options).then(function (publicURL) {
  ...
  (0, _tape2.default)('putFile & getFile unencrypted, not signed, with contentType', function (t) {
    ...
    putFileImpl(blockstack, path, fileContent, options).then(function (publicURL) {
      ...
      getFileImpl(blockstack, path, decryptOptions).then(function (readContent) {
  ...
  (0, _tape2.default)('putFile & getFile encrypted, not signed', function (t) {
    ...
    var _proxyquire6 = (0, _proxyquire19.default)('../../../lib/storage', {
      './hub': { getOrSetLocalGaiaHubConnection: getOrSetLocalGaiaHubConnection, uploadToGaiaHub: uploadToGaiaHub }
        ...
        putFileImpl = _proxyquire6.putFileImpl;

    var _proxyquire7 = (0, _proxyquire19.default)('../../../lib/storage', { // eslint-disable-line no-shadow
      './hub': { getOrSetLocalGaiaHubConnection: getOrSetLocalGaiaHubConnection, getFullReadUrl: getFullReadUrl }
    }),
        getFileImpl = _proxyquire7.getFileImpl;

    _fetchMock2.default.get(fullReadUrl, (0, _storage.encryptContentImpl)(blockstack, fileContent));
    var encryptOptions = { encrypt: true };
    var decryptOptions = { decrypt: true
      // put and encrypt the file
    };putFileImpl(blockstack, path, fileContent, encryptOptions).then(function (publicURL) {
      t.ok(publicURL, fullReadUrl);
    }).then(function () {
  ...
  (0, _tape2.default)('putFile & getFile encrypted, signed', function (t) {
    ...
    var decryptOptions = { decrypt: true, verify: true
      ...
      return getFileImpl(blockstack, 'file.json', decryptOptions).then(function (readContent) {
        t.equal(readContent, fileContent);
      });
    }).then(function () {
      return getFileImpl(blockstack, 'file.json', {
        decrypt: true,
        verify: true,
      ...
      return getFileImpl(blockstack, 'badPK.json', decryptOptions).then(function () {
      ...
      return getFileImpl(blockstack, 'badPK.json', {
        decrypt: true,
        verify: true,
        username: 'applejacks.id',
        app: 'origin'
      ...
      return getFileImpl(blockstack, 'badSig.json', decryptOptions).then(function () {
        return t.true(false, 'Should not successfully decrypt file');
      }).catch(function (err) {
  ...
  (0, _tape2.default)('putFile & getFile unencrypted, signed', function (t) {
    ...
    var multiplayerDecryptOptions = {
      ...
      app: 'origin'
      // put and encrypt the file
    };putFileImpl(blockstack, goodPath, fileContent, encryptOptions).then(function (publicURL) {
      ...
      return getFileImpl(blockstack, goodPath, decryptOptions).then(function (readContent) {
        t.equal(readContent, fileContent, 'should read the file');
      });
    }).then(function () {
      return getFileImpl(blockstack, badSigPath, decryptOptions).then(function () {
        return t.fail('Should have failed to read file.');
      }).catch(function (err) {
        return t.ok(err.message.indexOf('do not match ECDSA') >= 0, 'Should fail with complaint about bad signature');
      });
    }).then(function () {
      return getFileImpl(blockstack, noSigPath, decryptOptions).then(function () {
        return t.fail('Should have failed to read file.');
      }).catch(function (err) {
        return t.ok(err.message.indexOf('obtain signature for file') >= 0, 'Should fail with complaint about missing signature');
      });
    }).then(function () {
      return getFileImpl(blockstack, badPKPath, decryptOptions).then(function () {
        return t.fail('Should have failed to read file.');
      }).catch(function (err) {
        return t.ok(err.message.indexOf('match gaia address') >= 0, 'Should fail with complaint about matching addresses');
      });
    }).then(function () {
      return getFileImpl(blockstack, goodPath, multiplayerDecryptOptions).then(function (readContent) {
        t.equal(readContent, fileContent, 'should read the file');
      });
    }).then(function () {
      return getFileImpl(blockstack, badSigPath, multiplayerDecryptOptions).then(function () {
        return t.fail('Should have failed to read file.');
      }).catch(function (err) {
        return t.ok(err.message.indexOf('do not match ECDSA') >= 0, 'Should fail with complaint about bad signature');
      });
    }).then(function () {
      return getFileImpl(blockstack, noSigPath, multiplayerDecryptOptions).then(function () {
        return t.fail('Should have failed to read file.');
      }).catch(function (err) {
        return t.ok(err.message.indexOf('obtain signature for file') >= 0, 'Should fail with complaint about missing signature');
      });
    }).then(function () {
      return getFileImpl(blockstack, badPKPath, multiplayerDecryptOptions).then(function () {
        return t.fail('Should have failed to read file.');
      }).catch(function (err) {
        return t.ok(err.message.indexOf('match gaia address') >= 0, 'Should fail with complaint about matching addresses');
      });
  ...
  (0, _tape2.default)('promises reject', function (t) {
    ...
    var _proxyquire14 = (0, _proxyquire19.default)('../../../lib/storage', {
      './hub': {
        ...
        putFileImpl = _proxyquire14.putFileImpl;

    _fetchMock2.default.post('' + fullReadUrl, { status: 404, body: 'Not found.' });
    putFileImpl(blockstack, path, 'hello world', { encrypt: false }).then(function () {
  ...
  (0, _tape2.default)('putFile gets a new gaia config and tries again', function (t) {
    ...
    var _proxyquire15 = (0, _proxyquire19.default)('../../../lib/storage', {
      './hub': {
        ...
        putFileImpl = _proxyquire15.putFileImpl;
  ...
  (0, _tape2.default)('fetch404null', function (t) {
    ...
    blockstack.store.getSessionData().userData = {
      ...
      gaiaHubConfig: config // manually set for testing

    };_fetchMock2.default.get('' + config.url_prefix + config.address + '/foo.json', { status: 404 });

    var optionsNoDecrypt = { decrypt: false };
    (0, _storage.getFileImpl)(blockstack, 'foo.json', optionsNoDecrypt).then(function (x) {
      return t.equal(x, null, '404 should return null');
    });

    var optionsDecrypt = { decrypt: true };
    (0, _storage.getFileImpl)(blockstack, 'foo.json', optionsDecrypt).then(function (x) {
  ...
  (0, _tape2.default)('connectToGaiaHub', function (t) {
    ...
    var blockstack = new _lib.UserSession({ appConfig: appConfig });
    blockstack.store.getSessionData().userData = {
      appPrivateKey: privateKey // manually set for testing
  ...
  (0, _tape2.default)('listFiles', function (t) {
    ...
    blockstack.store.getSessionData().userData = {
      appPrivateKey: privateKey,
      gaiaHubConfig: gaiaHubConfig // manually set for testing
    ...
    var _proxyquire17 = (0, _proxyquire19.default)('../../../lib/storage', {
      './hub': { getOrSetLocalGaiaHubConnection: getOrSetLocalGaiaHubConnection }
        ...
        listFilesImpl = _proxyquire17.listFilesImpl;

    var files = [];
    listFilesImpl(blockstack, function (name) {
      files.push(name);
      return true;
    }).then(function (count) {
